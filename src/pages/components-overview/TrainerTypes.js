...
export default function TrainerType() {
  ...
  const navigate = useNavigate();
    ...
    const [dfield, setdfield] = useState(false);
    ...
    const AddTrainerType = () =>{

        // setAlert(true)
        var formdata = new FormData();
        formdata.append("ttype", Type);
        ...
        fetch("https://dashboard.weightlossondemand.com/backend/api/addtype", requestOptions)
        ...
        .then(result => {
            ...
            if(result.status == 200){
                setShow(false)
                setdialog(true)
                setdialogMsg("Type added successfully!")
            }else{
                setdfield(true)
    ...
    const handleDelete = (value) => {
        var requestOptions = {
          ...
          fetch(`https://dashboard.weightlossondemand.com/backend/api/delete_trtype/${value}`, requestOptions)
            .then(response => response.json())
            .then(result => {
                console.log(result)
                if(result.status === 200){
    ...
    const getVist = () => {
        axios({
            method: 'get',
            url: 'https://dashboard.weightlossondemand.com/backend/api/trainertype'
        ...
        selector: (row) => {
            return (
                <>
                    ...
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <a href="#" onClick={() => handleClickOpen(row.t_id)}>
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="16"
                            height="16"
    ...
    return (
        <ComponentSkeleton>
          ...
          <Link onClick={() => handleViewOpen()} style={textStyles.blink}>
            <Button style={textStyles.ibutton}>Add Trainer Type</Button>
            </Link> 
            ...
            <Alert  variant="filled" severity='error'>
                {dialogMsg}
                    ...
                    <p>{trName}</p> */}
                     ...
                     {/* <label style={textStyles.text} ><b>Trainer Name</b></label> */}
                    <TextField id="outlined-basic" label="Enter Trainer Type" variant="outlined" style={textStyles.text} value={Type} onChange={e => setType(e.target.value)}/>
                    {/* <label style={textStyles.text} > <b>Trainer Description</b></label> */}
                    <TextField id="outlined-basic" label="Enter Description" variant="outlined" style={textStyles.text} value={trDesc} onChange={e => settrDesc(e.target.value)}/>

                </DialogContent>
                <DialogActions>
                <Button onClick={AddTrainerType} style={textStyles.delete}>
                        Add
                    </Button>
